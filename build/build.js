...
var NONE = -1, STR = 'string', NUM = 'number', OBJ = 'object', FUN = 'function',
	...
	app = DATA.app, sys = DATA.sys, attr = sys.dom.attr, handler = sys.dom.handler,
	bs, ns = 'data-', templateData,
	json, log, detect, 
	Style, Css, domS, domInfo;
...
(function(){ //ut prototype
	var es5 = Object.create ? 1 : 0, r = {}, fn = Object.prototype,
		bsImmutable = es5 ? function (){
	...
	if(es5) Object.defineProperty(fn, 'bsImmutable', (r.value = bsImmutable, r));
	else fn.bsImmutable = bsImmutable;
	fn = Array.prototype;
	Object.prototype.bsImmutable('bsSlice', fn.slice, 'bsJoin', fn.join);
	String.prototype.bsImmutable(
		'isNumber', function(){return parseFloat(this) + '' == this;},
		'ex', (function(){
			var self, prefix = {}, r = [
				'{', /\{\^([^\^]+)\^\}/g, function(_0, v){
					var k = v.charAt(0);
					if(k == '$') return templateData[v.substr(1)];
					if(self === prefix) return v;
					else return bs(k == '@' ? domInfo(self).prefix.ex(prefix) + v.substr(1) : v);
				},
				'@', /[@]([^@]+)[@]/g, function(_0, _1){
					return '[^this["' + _1 + '"]^]';
				},
				'[', /\[\^([^\^]+)\^\]/g, function(_0, _1){
					return (new Function('bs', 'return (' + _1 + ');')).call(self, bs);
				}], rl = r.length;
			return function(target){
				var v = this, a = arguments, arg, i, j;
				if(typeof target == STR && a.length > 1){
					arg = {}, i = 0, j = a.length;
					while(i < j) arg[a[i++]] = a[i++];
					target = arg;
				}
				for(self = target, i = 0, j = rl; i < j; i += 3) if(v.indexOf(r[i]) != NONE) v = v.replace(r[i+1], r[i+2]);
				return v + '';
			};
		})()
	),
	Array.prototype.bsImmutable('trim', function(isNew){
...
(function(){
	...
	if(!fn.toISOString) fn.bsImmutable(
	...
	if(!performance.now) performance.bsImmutable('now',Date.now ? function(){return Date.now() - offset;} :
		function(){return +new Date - offset;}
	);
	if(!Date.now) Date.bsImmutable('now', function(){return +new Date;});
})();
bs.bsImmutable('JSON', json = {
	parse:function(v){
		var f = new Function('', 'return ' + v.trim());
		return f();
	},
	stringify:(function(){
		var stack = {length:0}, r0 = /["]/g, Deter = function(v){this.v = v;}, Key = function(){}, pool = {length:0},
			sep = new Deter(','), a0 = new Deter('['), a1 = new Deter(']'), o0 = new Deter('{'), o1 = new Deter('}');
		return JSON.stringify || function(v){
			var r = '', i, k;
			stack.length = 0;
			do{
				...
				case'object':
					if(!v) r += 'null';
						...
						while(i--) stack[stack.length++] = v[i], stack[stack.length++] = sep;
						stack[stack.length - 1] = a0;
					}else{
						stack[stack.length++] = o1;
						for(i in v) if(v.hasOwnProperty(i)){
							k = pool.length ? pool[--pool.length] : new Key, k.v = i,
							stack[stack.length++] = v[i], stack[stack.length++] = k, stack[stack.length++] = sep;
						}
						stack[stack.length - 1] = o0;
					}
				}
			}while(stack.length && (v = stack[--stack.length]))
			return r;
		};
	})()
});
Object.prototype.bsImmutable('clone', function(){
	if(this && typeof this == OBJ) return json.parse(json.stringify(this));
	return this;
});
if(!W['JSON']) W['JSON'] = json;
if(doc){
	if(!('firstElementChild' in document.documentElement) || !('nextElementSibling' in document.documentElement))(function(){
		var v = {}, f = function(){
				var a = arguments, i = 0, j = a.length;
				while(i < j) Object.defineProperty(Element.prototype, a[i++], (v.get = a[i++], v));
			},
			mk = function(k0, k1){
				return function(){
					var el = this[k0];
		...
		if(!('firstElementChild' in document.documentElement)) Object.defineProperty(Element.prototype, 'firstElementChild', (v.get = mk('firstChild', 'nextSibling'), v));
		if(!('lastElementChild' in document.documentElement)) Object.defineProperty(Element.prototype, 'lastElementChild', (v.get = mk('lastChild', 'previousSibling'), v));
		if(!('previousElementSibling' in document.documentElement)) Object.defineProperty(Element.prototype, 'previousElementSibling', (v.get = mk('previousSibling', 'previousSibling'), v));
		if(!('nextElementSibling' in document.documentElement)) Object.defineProperty(Element.prototype, 'nextElementSibling', (v.get = mk('nextSibling', 'nextSibling'), v));
		if(!('children' in document.documentElement)) Object.defineProperty(Element.prototype, 'children', (v.get = 'children', function(){
...
http = (function(){
	...
	return function http(method, end, U, arg){
		var param, cnt, curr, loaded, r, rsaurl, rsaTarget,
		...
		if(j > 3){
			...
			while(i < j){
				...
				case'@body':body = typeof v == 'string' ? encodeURIComponent(v) : v; break;
				...
				default:
					...
					else send[send.length] = encodeURIComponent(k) + '=' + encodeURIComponent(v);
				}
			}
			if(send.length) body = send.join('&');
		}
		//URL처리
		u = U.trim().split('#');
		U = u[0];
...
(function(){
	...
	})() : "XMLHttpRequest" in W ? function( data, end ){
		var x = async(end);
...
domInfo = (function(){
	var r = {};
	return function(el){
		var a = el.attributes, group, v = a[ns + 'g'],
			target, ta, prefix, lock;
		if(!v){
			target = el;
			while((target = target.parentNode) && (ta = target.attributes)){
				if(v = ta[ns + 'group']){
					group = v.value, prefix = a[ns + 'prefix'] || ta[ns + 'prefix'], lock = a[ns + 'lock'] || ta[ns + 'lock'];
					break;
				}
			}
			if(!group) group = '__root__', prefix = a[ns + 'prefix'], lock = a[ns + 'lock'];
			el.setAttribute(ns + 'g', v = group + '~' + (prefix ? prefix.value + '.' : '') + '~' +(lock ? lock.value : '')),
			el.S = domS;
		}else v = v.value;
		if(r[v]) return r[v];
		v = v.split('~');
		return r[v] = {group:v[0], prefix:v[1], lock:v[2]};
	};
})(),
domS = (function(){
	...
	util = {
		...
		render:function(el, k, v){
			var data = el.__renderData__;
			if(data) el.__renderData__ = null;
			else if(data = el.getAttribute(ns + 'd')){
				data = data.ex(el)
			}else return;
			bs.render(el, v, data);
		},
		renderData:function(el, k, v){el.__renderData__ = v;}
	};
	return function(){
		var prefix, a = arguments, i = 0, j = a.length, k, v, f, info;
		if(a[0] === null) return del(this);
		while(i < j){
			if(!attr[k = a[i++]]){ //k를 종류에 따라 분리함
			...
			f = attr[k];
			if(i == j) switch(f){
			case STYLE:return (this.bsStyle || (this.bsStyle = new Style(this.style))).g(k);
			case THIS:return this;
			default:return f != NONE ? f(this, k) : null;
			}
			v = a[i++];
			if(typeof v == STR){
				v = v.ex(this);
			...
			if(f == STYLE) (this.bsStyle || (this.bsStyle = new Style(this.style))).s(k, v);
			else if(f != NONE) f(this, k, v);
		}
		return v;
	};
})();
...
var attach = {}, //IE 중복 리스너
	...
	on = eOn = W['addEventListener'] ? function(el, type, listener){el.addEventListener(type, listener);} :
		W['attachEvent'] ? function(el, type, listener){
			...
			for(t0 = attach[type], i = 0, j = t0.el.length; i < j; i++){
				if(t0.el[i] == el && t0.li[i] == listener) return
			}
			t0.el.push(el), t0.li.push(listener);
	...
	off = eOff = W['removeEventListener'] ? function(el, type, listener){el.removeEventListener(type, listener);} :
		W['detachEvent'] ? function(el, type, listener){
			...
			if(t0 = attach[type]){
				i = t0.el.length
				while(i--){
					if(t0.el[i] == el && t0.li[i] == listener) t0.el.splice(i, 1), t0.li.splice(i, 1);
				}
			}
	...
	handleEvent = (function(){
		var cache = {}, ls = {},
			...
			keyName = KEY.code2name,
			keyCode = KEY.name2code,
			lock = {};
			ev = {__pos__:0};
			ev.bsImmutable(
				'wheelDelta', function(){
					var e = this.event, d = e.detail, w = e['wheelDelta'] ? e.wheelDelta : -e.deltaY * 20,
						n = 225, n1 = n - 1;
				...
				'isLeave', function(){
					var pos = this.pos(), el;
					if(bs.DETECT.browser == 'zombie') return false;
				...
				'unlock', function(){lock[this.__lock0__] = 0;},
				'unlockGroup', function(){lock[this.__lock1__] = 0;},
				'group', (function(){
					var stack = {length:0},
						groups = {
							'__root__':{el:doc.body, name:'__root__', find:{}}
						},
						init = function(){
							...
							if(!t.__group__){
								i = t; 
								do{
									if(j = i.attributes[ns + 'group']){
										j = j.value;
										if(k = groups[j]){
											if(k.el != i) return err('ev.group.init');
										}else{
											groups[j] = k = {
												el:(i.S = domS, i),
												name:j,
								...
								if(!k) t.__group__ = groups['__root__'];
							}
							return t.__group__;
						},
						keys = {},
						kS = function(){
							var i = this.length;
							while(i--) domS.apply(this[i], arguments);
						};
					return {}.bsImmutable(
						'render', function(data){
							var target = init(), idx = target.name, d, el, a, i, j, k, v;
							if(idx.indexOf('|') == NONE) return;
							idx = idx.split('|');
							if(!data) data = idx[0];
							d = bs(data)[idx = idx[1]];
							a = arguments, i = 1, j = arguments.length;
							while(i < j){
								k = a[i++];
								if(i == j) return d[k];
								v = a[i++];
							...
							bs.render(target.el, null, data + '.' + idx);
						},
						'el',function(){
							return init().el
						},
						'S', function(){
							return domS.apply(init().el, arguments);
						},
						'data', function(k, v){
							var el = init().el;
						...
						'name', function(){return init().name;},
						'key', function(){
							var k = init().name + ':' + Array.prototype.join.call(arguments, ','), r, i;
							if(!(r = keys[k])){
								keys[k] = r = [], r.S = kS;
								i = arguments.length;
								while(i--) r[i] = this.find('key', arguments[i]);
							}
							return r;
						},
						'find', function(k){
							...
							group = init(),
							i = k.charAt(0) == '@' ? (k = k.substr(1), 1) : 0,
							v = arguments[1];
							if(!i && (el = group.find[k+':'+v])) return el;
							el = group.el.firstElementChild;
							i = stack.length = 0;
							do{
		...
		return function(e){
			var cacheKey, type0, type, el, a, info, prefix, group, listeners, listener, lock0, lock1, i, j, k, v, m, n;
			if(!e) e = event;
			el = e.target || e.srcElement, type = e.type;
			if(!el) return console.log('e.target,e.srcElement가 없어서 무시됨');
			if(el == W || el === doc){
				if(listeners = sys.sysEv[type]){
					ev.event = e, ev.target = el, ev.type = type;
					for(i = 0, j = listeners.length; i < j; i++) if(listeners[i].call(W, ev)) break;
				}
			}else do{
				a = el.attributes;
				//이벤트에 해당되는 키확인(가상키포함) : data-click, data-down(가상)
				type0 = 0;
				if(listeners = a[ns + type] || (type0 = e2v[type]) && a[ns + type0]){
					if(!type0) type0 = type;
					listeners = listeners.value;
					if(i = a[ns + 'lock']){
						if(lock[lock0 = i.value]) return;
						lock[lock0] = 1;
					}
					//그룹정보에 대한 캐쉬를 생성함.
					info = domInfo(el);
					if(lock1 = info.lock){ //락을 적용함
						if(lock[lock1]) return;
						lock[lock1] = 1;
					}
					//ev객체 초기화
					ev.event = e,
					...
					ev.type = type,
					ev.prefix = info.prefix,
					ev.keyName = keyName[ev.keyCode = e.keyCode],
					ev.__lock0__ = lock0,
					ev.__lock1__ = lock1,
					ev.__group__ = info.group,
					ev.__attr__ = a,
					ev.__pos__ = 0;

					if(posCat[type] == 1 || posCat[type] == 4) ev.pos();

					cacheKey = el.getAttribute(ns + 'cache');
					listeners = ls[listeners] || (ls[listeners] = listeners.split(',').trim());
					i = 0, j = listeners.length;
					while(i < j){
						k = listeners[i++];
						if(!(listener = cache[k])){
							k = k.ex(el),
							listener = bs(k.charAt(0) == '@' ? k : info.prefix + k);
							if(cacheKey && listener && typeof listener == FUN) cache[k] = listener;
						}
						//리스너가 없거나 리스너가 참을 반환하면 정지
						if(!listener || listener.call(el, ev)) break;
					}
					break;
				}
			}while((el = el.parentNode) && el.parentNode && !a[ns + 'group'])
		};
	})(),
	bindTarget = (function(){
		var bind = {click:doc};
		return bs.DETECT.browser == 'zombie' ? function(type){
			return bind[type] || W;
		}:function(type){
			return bind[type] || ('on' + type in W ? W : 'on' + type in doc ? doc : 0);
		};
	})();
bs.bsImmutable(
...
if(!W['onorientationchange']) (function(){
	var listener = function(e){handleEvent(E);},
		E = {target:W, type:'orientationchange'};
	v2e['orientationchange'] = function(isOn){
		if(isOn) on(W, 'resize', listener);
		else off(W, 'resize', listener);
	};
})();
if(!W['onhashchange']) v2e['hashchange'] = (function(){
	var on, old,
		E = {target:W, type:'hashchange'},
		f = function(){
			if(!on) return;
	...
	return function(isOn){
		if(on = isOn) old = location.hash, requestAnimationFrame(f);
	};
})();
if(!('onscroll' in W)) v2e['scroll'] = (function(){
	...
	return function(isOn){
		if(on = isOn) oldX = W.pageXOffset || docel.scrollLeft|| 0, oldY = W.pageYOffset || docel.scrollTop || 0, requestAnimationFrame(f);
	};
})();
	...
	'validate', (function(){
		...
		return function(el, group, partial){
			...
			if(!(cacheKey = el.getAttribute('data-cache')) || !cache[cacheKey] || !(v = cache[cacheKey][group])){
				...
				if((v = el.getAttribute('data-vali')) && (v = v.trim()) && domInfo(el).group == group){
					i = v.indexOf(','),
...
bs.css('.dataTemplate,[data-template]{display:none}'),
bs.bsImmutable(
'render', (function(){
	var r = /\,/g, cache = {}, template = {}, rendered = {}, isScaned = 0,
		stack = {length:0}, st = {length:0},
		fragP = {length:0},
		frag = function(v){
			if(v) fragP[fragP.length++] = v;
			else return fragP.length ? fragP[--fragP.length] : doc.createDocumentFragment();
		},
		idx = function(el){
			var i = 0;
			while(el = el.previousSibling) i++;
		...
		param = function(v){
			v = v.split(','), v[v.length] = '@' + ns + 'g', v[v.length] = null;
			return v;
		};
	return function(el, tmpl, data){
		var prevData, cacheKey, target, el0, el1, tmp, loc, f, v, r, i, j, k, l, m, n;
		if(typeof el == STR) el = docId(el);
		if(data){
			if(data.charAt(0) == '$'){
				data = templateData[data.substr(1)];
		...
		if(tmpl){
			if(!isScaned){
				isScaned = 1;
				el0 = doc.body, stack.length = st.length = i = 0;
				do{
					if(v = el0.firstElementChild) stack[stack.length++] = v;
					if(v = el0.nextElementSibling) stack[stack.length++] = v;
					if(v = el0.getAttribute(ns + 'template')){
						template[st[st.length++] = v] = tmp = {_root_:el0, _data_:v, length:0},
						el0.S = domS,
						el0.S('class-', 'dataTemplate', '@' + ns + 'template', null);
						el0.parentNode.removeChild(el0);
						if(v = el0.S('@' + ns + 's')) tmp[tmp.length++] = {el:[], v:param(v)};
					}
				}while(stack.length && (el0 = stack[--stack.length]))
				i = st.length;
				while(i--){
					tmp = template[st[i]], el0 = tmp._root_;
					if(el1 = el0.firstElementChild){
						stack.length = 0;
						do{
							if(v = el1.firstElementChild) stack[stack.length++] = v;
							if(v = el1.nextElementSibling) stack[stack.length++] = v;
							if(v = el1.getAttribute(ns + 's')){
								loc = {length:0}, k = el1;
								while(k.parentNode && k.parentNode.nodeType == 1) loc[loc.length++] = idx(k), k = k.parentNode;
								tmp[tmp.length++] = {el:loc, v:param(v)};
							}
						}while(stack.length && (el1 = stack[--stack.length]))
					}
				}
			}
			if(!data) data = bs(tmpl.ex(v._root_));
			if(templateData) prevData = templateData;
			if(!(tmp = el.children.length) || !(i = el.firstElementChild.getAttribute(ns + 'group')) || i.indexOf('|') == NONE){
				tmp = 0;
				try{el.innerHTML = '';}catch(e){ if(tmp) while(tmp--) el.removeChild(el.children[tmp]);}
			}
			if((j = data.length) > tmp) f = frag();
			for(i = 0; i < j; i++){
				templateData = data[i], k = tmpl == '*' ? templateData['@template'] : tmpl;
				if(!(v = template[k])) return templateData = null;
				n = v.length;
				if(i < tmp) k = el.children[i];
				else f.appendChild(k = v._root_.cloneNode(true));
				k.setAttribute(ns + 'group', k + '|' + i);
				m = 0;
				while(m < n){
					el0 = k, target = v[m++], el1 = target.el, l = el1.length;
					if(l) while(l-- && el0) el0 = el0.childNodes[el1[l]];
					if(el0) domS.apply(el0, target.v);
				}
			}
			templateData = null;
			if(f) el.appendChild(f), frag(f);
			if(tmp > j) while(tmp-- > j) el.removeChild(el.lastElementChild);
			if(prevData) templateData = prevData;
		}else{
			...
			if(!(cacheKey = el.getAttribute(ns + 'cache')) || !(r = cache[cacheKey])){
				if(el.attributes[ns + 'template']) return;
				if(cacheKey) r = {length:0};
				if(v = el.getAttribute(ns + 's')){
					domS.apply(el, v = v.split(','));
					if(cacheKey) r[r.length++] = el, r[r.length++] = v;
				}
				tmp = el.getElementsByTagName("*"), j = tmp.length;
				while(j--){
					el1 = el0 = tmp[j], k = 1;
					do{
						if(el0.getAttribute(ns + 'template') || ((l = el0.getAttribute(ns + 'group')) && l.indexOf('|') != NONE)){
							k = 0;
							break;
						}
					}while((el0 = el0.parentNode) && el0 != el)
					/*
					if(l = el1.getAttribute(ns + 'keydown')){
						eOn(el1, 'keydown', bs(l))
					}else if(l = el1.getAttribute(ns + 'keyup')){
						eOn(el1, 'keyup', bs(l))
					}
					*/
					if(k && (v = el1.getAttribute(ns + 's'))){
						domS.apply(el1, v = v.split(','));
						if(cacheKey) r[r.length++] = el1, r[r.length++] = v;
					}
				}
				if(cacheKey) cache[cacheKey] = r;
			}else{
				i = 0, j = r.length;
				while(i < j) domS.apply(r[i++], r[i++]);
			}
			if(data){
				templateData = null;
				if(prevData) templateData = prevData;
...
(function(){
	var TOP = W.top, isBlock, isOpen, currWin, winID = 0, maxId, delIDs = [],
		history = [], change,
		back = function(){TOP.history.back();},
		go = function(v){TOP.location.href = v;},
		start = function(){
			currWin.S('transform', currWin.S('*windowStart')),
			isBlock = 0;
		},
		open = function(url, init, st){
			isOpen = 1,
			currWin = docNew('iframe'),
			currWin.S('display', 'block', 'class', 'bsWindow', '@src', url, '@id', 'bsWindow'+ winID,
				'transform', init, '*windowInit', init, '*windowStart', st,
				'<', doc.body),
			requestAnimationFrame(start),
			maxId = ++winID;
		},
		remover = function(){
			var i = delIDs.length, win;
			while(i--) if(win = docId('bsWindow' + delIDs[i])) win.S(null);
			isBlock = 0;
		};
	bs.css('html,body{margin:0;padding:0;height:100%}.bsWindow{transition:all 0.4s;position:fixed;border:0;margin:0;left:0;right:0;bottom:0;top:0;padding:0;width:100%;height:100%}'),
	bs.bsImmutable(
	'urlType', function(type){
		switch(type||'hash'){
		case'pushState':
			bs.systemEvent('popstate', function(e){
				var state = e.event.state, hash, target, i, j, win;
				for(i = state ? +state.id + 1 : 0; i < maxId; i++){
					delIDs[delIDs.length] = i,
					win = docId('bsWindow' + i),
					win.S('transform', win.S('*windowInit'));
				}
				setTimeout(remover, 450);
			}),
			go = function(url, init, st){
				history.pushState({id:winID}, url, url),
				open(url, init, st);
			};
			break;
		case'hash':
			bs.systemEvent('hashchange', change = (function(){
				var del = function(i, j){
						var id, win;
						while(i < j) delIDs[delIDs.length] = id = history[i++].id,
							(win = docId('bsWindow' + id)).S('transform', win.S('*windowInit'));
						setTimeout(remover, 450);
					};
				return function(e){
					var hash, target, id, i, j, k, win;
					if(isOpen) return isOpen = 0;
					hash = location.hash,
					i = history.length;
					if(!hash || hash == '#'){
						if(i) del(0, i), history.length = 0;
					}else if(hash = bs.decode(hash.substr(1))){
						j = i;
						while(i--) if(history[i].url == hash){
							if(++i < j) del(i, j), history.length = i;
							return;
						}
						location.href = hash;
					}
		...
		while(i < j){
			...
			if(i == j){
				if(t0 = localStorage.getItem(k)){
					...
					//if(t0.charAt(0) == '@') t0 = json.parse(decomp(t0.substr(1)));
			...
			if(v === null){
				...
				//t0 = (v && typeof v == OBJ ? '@' + comp(json.stringify(v)) : v) + '', n = 0;
